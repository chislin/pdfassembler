...
export type BinaryFile = Buffer | ArrayBuffer | TypedArray;
...
export class PDFAssembler {
    ...
    constructor(inputData?: BinaryFile | Object, userPassword = '') {
        ...
        if (typeof inputData === 'object') {
            if (inputData instanceof Buffer || inputData instanceof ArrayBuffer || inputData instanceof Uint8Array) {
    ...
    toArrayBuffer(file: BinaryFile): Promise<ArrayBuffer|Buffer> {
        const typedArrays = [
        ...
        return file instanceof ArrayBuffer
            ? Promise.resolve(file) : typedArrays.some(typedArray => file instanceof typedArray)
                ? Promise.resolve(<ArrayBuffer>(<TypedArray>file).buffer) : file instanceof Buffer
                    ? Promise.resolve(file) : Promise.resolve(new ArrayBuffer(0));
    }
    ...
    assemblePdf(nameOrOutputFormat = 'output.pdf'): Promise<ArrayBuffer | Uint8Array> {
